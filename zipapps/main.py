...
def get_pip_main(ensurepip_root: str | None = None):
    try:
        import pip
    except ImportError:
        import ensurepip

        assert ensurepip.bootstrap(root=ensurepip_root) == 0
        if ensurepip_root:
...
class ZipApp(object):
    ...
    def __init__(
        ...
        unzip: str = "",
        unzip_path: str = "",
        ignore_system_python_path: bool = False,
        main_shell: bool = False,
        ...
        self._output_path: Path = Path(self.output)
        ...
        self.pip_args: list[str] | None = pip_args
    ...
    def create_archive_layer(self):
        if self.compressed:
            ...
            compresslevel = 0
        _kwargs: dict[str, Any] = dict(mode="w", compression=compression)
        if self._is_greater_than_python_37:
            _kwargs["compresslevel"] = compresslevel
        with ZipFile(self._output_path, **_kwargs) as zf:
    ...
    def prepare_entry_point(self):
        # reset unzip_names
        unzip_names: set[str] = set(self.unzip.split(",")) if self.unzip else set()
        warning_names: dict[str, dict[Any, Any]] = {}
    ...
    def prepare_active_zipapps(self):
            ...
            if self.main:
                if re.match(r"^\w+(\.\w+)?(:\w+)?$", self.main):
                    ...
                    if module:
                        ...
                        self._log(
                            rf"[INFO]: -m: matches re.match(r'^\w+(\.\w+)?(:\w+)?$', self.main), add as `{runner}`."
                        )
                        return runner
                else:
                    self._log(
                        rf"[INFO]: -m: not matches re.match(r'^\w+(\.\w+)?(:\w+)?$', self.main), add as raw code `{self.main}`."
                    )
                    return self.main
            return ""
        ...
        code = get_data(__name__, "entry_point.py.template")
        assert code is not None
        code = code.decode("u8")
        (self._cache_path / "__main__.py").write_text(code.format(**kwargs))

        code = get_data(__name__, "ensure_zipapps.py.template")
        assert code is not None
        code = code.decode("u8")
        (self._cache_path / "ensure_zipapps.py").write_text(code.format(**kwargs))

        code = get_data(__name__, "activate_zipapps.py")
        assert code is not None
        code = code.decode("u8")
    ...
    def pip_install(self):
        ...
        _pip_args: list[str] = (
            ["install", "--target", target] + self.pip_args
            if self.pip_args
            else ["install", "--target", target]
        )

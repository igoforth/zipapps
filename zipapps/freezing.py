...
def ttime(
    ...
    tzone: int = int(-time.timezone / 3600),
    fail: str = "",
    fmt: str = "%Y-%m-%d %H:%M:%S",
):
    fix_tz = tzone * 3600
...
class FreezeTool(object):
    ...
    def __init__(self, output: str, pip_args: list[str]):
        if not pip_args:
            raise RuntimeError("pip args is null")
        self.temp_dir: tempfile.TemporaryDirectory[str] | None = None
        self.pip_args: list[str] = pip_args
    ...
    def freeze_requirements(self, output: str):
        if self.output_path == "-":
    ...
    def __exit__(self, *e):  # type: ignore
        self.remove_env()
